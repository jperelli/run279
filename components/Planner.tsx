...
export default function Planner() {
  ...
  const [total, setTotal] = useState(
    partials.reduce((acc, partial) => acc + partial.pace, 0)
  );
  ...
  useEffect(() => {
    ...
    if (totalLock) {
      const touchedPartialIndex =
        prevPartials?.findIndex(
          (prevPartial, i) => prevPartial.pace !== partials[i].pace
        ) || 0;
      if (touchedPartialIndex !== -1) {
        const diff =
          (prevPartials?.[touchedPartialIndex]?.pace || 0) -
          partials?.[touchedPartialIndex]?.pace;
        const partialAvg = diff / (length - 1);
        setPartials(
          partials.map((partial, i) =>
            i === touchedPartialIndex
              ? partial
              : { ...partial, pace: partial.pace + partialAvg }
          )
        );
      }
    }
  ...
  useEffect(() => {
    if (!totalLock) {
      const diff =
        partials.reduce((acc, partial) => acc + partial.pace, 0) - total;
      const partialAvg = diff / length;
      setPartials(
        partials.map((partial) => ({
          ...partial,
          pace: partial.pace + partialAvg,
        }))
      );
    }
  }, [total]);
                ...
                <input
                  ...
                  onChange={(e) =>
                    setPartials(
                      partials.map((p, i) =>
                        i === partial.km - 1
                          ? { ...p, pace: Number(e.target.value) / 60 }
                          : p
                      )
                    )
                  }
          ...
          <tr>
            ...
            <td>
              <input
                type="checkbox"
                checked={totalLock}
                onChange={(e) => setTotalLock(e.target.checked)}
              />
            </td>
          </tr>
          <tr>
